...
// Интерфейс для результата вычисления
interface CalculationResult {
    parameters: FormulaParameters;
    result: number | Complex;  // Может быть действительным или комплексным
    error: number;             // Погрешность в процентах
    isValid: boolean;
...
const MIN_ACCURACY: number = 1e-8;
const MAX_ACCURACY: number = 1e-7;

// Выбранное фиксированное значение погрешности (в середине диапазона)
const FIXED_ERROR_PERCENT: number = 5e-8; // 5×10^-8% = 0.00000005%
 ...
 */
function validateParameter(value: number, paramName: keyof typeof PARAMETER_RANGES): { isValid: boolean; correctedValue?: number; message?: string } {
    ...
    if (value < range.min || value > range.max) {
        const correctedValue: number = Math.max(range.min, Math.min(range.max, value));
        return { 
            isValid: false, 
            correctedValue, 
    ...
    if (range.step > 0) {
        ...
        if (Math.abs(value - expectedValue) > tolerance) {
            const correctedValue: number = expectedValue;
            return { 
                isValid: false, 
                correctedValue, 
 ...
 */
function calculateFormulaInternal(params: FormulaParameters): number | Complex {
 ...
 */
function analyzeAccuracyIssues(error: number, result: number | Complex): string {
...
function performCalculation(inputParams: Partial<FormulaParameters>): CalculationResult {
    ...
    for (const [key, value] of Object.entries(inputParams)) {
        if (value !== undefined && value !== null) {
            const validation = validateParameter(Number(value), key as keyof typeof PARAMETER_RANGES);
    ...
    let finalResult: number | Complex;
    if (typeof result === 'number') {
        finalResult = Number(result.toFixed(10));
    } else {
        // Комплексное число - округляем компоненты
        finalResult = new Complex(
            Number(result.real.toFixed(10)),
            Number(result.imag.toFixed(10))
        );
    }
 ...
 */
function calculateFormulaFromHTML(): void {
    const inputs = {
        a: (document.getElementById('inputA') as HTMLInputElement).value,
        c: (document.getElementById('inputC') as HTMLInputElement).value,
        d: (document.getElementById('inputD') as HTMLInputElement).value,
        x: (document.getElementById('inputX') as HTMLInputElement).value,
    ...
    const numericInputs: Partial<FormulaParameters> = {};
    for (const [key, value] of Object.entries(inputs)) {
        if (value && value.trim() !== '') {
            numericInputs[key as keyof FormulaParameters] = parseFloat(value);
        }
    }
    
    
    // Проверка на заполненность всех полей
    const requiredFields: (keyof FormulaParameters)[] = ['a', 'd', 'x', 'y', 'z'];
...
function clearErrors(): void {
    const errorElements: string[] = ['errorA', 'errorD', 'errorX', 'errorY', 'errorZ'];
 ...
 */
function loadTestData(event: Event): void {
 ...
 */
function loadControlData(event: Event): void {
    const file = (event.target as HTMLInputElement).files?.[0];
    if (file) {
        const reader = new FileReader();
        reader.onload = function(e) {
 ...
 */
function processTestData(data: any, type: string): void {
 ...
 */
function displayBatchResults(results: CalculationResult[], type: string): void {
    const tbody = document.getElementById('resultsBody') as HTMLTableSectionElement;
    tbody.innerHTML = '';
